...
import (
	...
	"github.com/joshuatcasey/libreload-beta"
	"github.com/joshuatcasey/libreload-beta/watchexec"
...
func testWatchexec(t *testing.T, context spec.G, it spec.S) {
	var (
		...
		watchexecReloader watchexec.WatchexecReloader
	)

	it.Before(func() {
		watchexecReloader = watchexec.NewWatchexecReloader()
	})

	it("is a libreload.Reloader", func() {
		var _ libreload.Reloader = watchexec.NewWatchexecReloader()
	})
		...
		context("When it should enable live reload", func() {
			context("when BP_LIVE_RELOAD_ENABLED is 'true'", func() {
				...
				it("should require watchexec at launch", func() {
					require, err := watchexecReloader.ShouldEnableLiveReload()
		...
		context("failure cases", func() {
			context("when BP_LIVE_RELOAD_ENABLED is not a valid boolean value", func() {
				...
				it("should return an error", func() {
					_, err := watchexecReloader.ShouldEnableLiveReload()
	...
	context("TransformReloadableProcesses", func() {
		var (
			...
			reloadableProcessSpec libreload.ReloadableProcessSpec
		...
		it("should transform the original process into non-reloadable and reloadable processes", func() {
			nonReloadableProcess, reloadableProcess := watchexecReloader.TransformReloadableProcesses(originalProcess, reloadableProcessSpec)
